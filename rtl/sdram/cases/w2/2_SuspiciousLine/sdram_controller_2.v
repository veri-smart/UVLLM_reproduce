...
module sdram_controller (
  ...
  always @(posedge clk)
    if (~rst_n) refresh_cnt <= 10'b0; else if (state == REF_NOP2) refresh_cnt <= 10'b0;
    // Removed redundant condition
    else if (state != REF_PRE && state != REF_NOP1 && state != REF_REF && state != REF_NOP2) refresh_cnt <= refresh_cnt + 1'b1;



  always @(*) begin
    if (state == WRIT_CAS) {data_mask_low_r, data_mask_high_r} = 2'b00;
    ...
    if (state == READ_ACT | state == WRIT_ACT) begin
      ...
      addr_r = {
        {SDRADDR_WIDTH - (11) {1'b0}}, 1'b1, {10 - COL_WIDTH{1'b0}}, haddr_r[COL_WIDTH-1:0]
      };
