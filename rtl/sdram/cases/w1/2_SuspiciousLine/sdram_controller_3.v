...
module sdram_controller (
  ...
  reg [15:0] data_out_r; always @(posedge clk or negedge rst_n) data_out_r <= wr_data_r; assign data_out = data_out_r;
  ...
  always @(posedge clk or negedge rst_n)
    if (~rst_n) begin
      ...
      if (wr_enable & ~busy) wr_data_r <= wr_data;

      if (state == READ_READ) begin
        rd_data_r  <= data_in;
        rd_ready_r <= 1'b1;
      end else if (rd_ready_r) begin
        rd_ready_r <= 1'b0;
      end
