...
module sdram_controller (
  ...
  assign data_out = (state == WRIT_CAS) ? wr_data_r : 16'b0;
  ...
  always @(posedge clk)
    if (~rst_n) begin
      ...
      state_cnt <= (state_cnt == 0) ? state_cnt_nxt : state_cnt - 1'b1;
      

      if (wr_enable & ~busy) wr_data_r <= wr_data; else if (state == WRIT_CAS) wr_data_r <= wr_data_r;
