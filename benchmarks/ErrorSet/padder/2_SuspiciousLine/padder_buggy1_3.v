module padder (
  ...
  assign buffer_full = done ? 1'b0 : i[17];
  assign out_ready = done ? 1'b0 : i[17];
  assign accept = in_ready & ~buffer_full & ~state;
  assign update = in_ready & ~i[17] & ~state;
  ...
  always @(posedge clk)
    if (reset) i <= 0;
    else if (f_ack) i <= 0; else if (update) i <= i + 1;
