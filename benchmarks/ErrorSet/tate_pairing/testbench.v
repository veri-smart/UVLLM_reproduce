...
task check_results;
begin
    if (done !== ref_done || out !== ref_out) begin
        $fdisplay(log_file, "FAIL: Test case %d, Cycle %d", test_case, cycle_count);
        $fdisplay(log_file, "  Inputs: x1=%h, y1=%h, x2=%h, y2=%h", x1, y1, x2, y2);
        $fdisplay(log_file, "  Expected: done=%b, out=%h", ref_done, ref_out);
        $fdisplay(log_file, "  Actual:   done=%b, out=%h", done, out);
...
// Wait for pairing computation to complete
...
begin
    ...
    if (timeout_cycles >= 200000) begin
        $display("ERROR: Timeout waiting for pairing completion");
        fail_count = fail_count + 1;
    end
end
endtask
...
// Reference model for Tate Pairing (simplified version for comparison)
...
module ref_duursma_lee_algo (
  ...
  wire [`W6:0] g, v7, v8;
  wire [`WIDTH:0] mu, nmu, ny, x, v2, v3, v4, v5, v6;
  wire [1:0] v9;
...
module ref_f36m_mult (
  ...
  wire [`W2:0] a0, a1, a2,
                 ...
                 d0, d1, d2, d3, d4;
  reg [6:0] K;
  wire e0, e1, e2, e3, e4, e5, mult_done, p, rst;
  wire [`W2:0] in0, in1;
  wire [`W2:0] o;
...
module ref_f36m_cubic (
  ...
  always @(posedge clk) c <= {c2, c1, c0};
endmodule


module ref_second_part (
    clk,
    reset,
    a,
    c,
  ...
  reg [3:0] K;
  wire [`WIDTH:0] d0, d1, d2, d3, d4, d5, c0, c1, c2, c3, c4, c5;
  wire [`W3:0] a0, a1, b0, b1, v1, v2, v3, v4, v5, v6, v7, v8, nv6;
  wire [1:0] v9, v10;
  ...
  always @(posedge clk)
    if (reset) K <= 4'b1000;
    else if ((K[3] & rst2) | (K[2] & rst3) | (K[1] & done3) | K[0]) K <= K >> 1;
...
module ref_f33m_mux2 (
  ...
  genvar i;
  generate
    for (i = 0; i <= `W3; i = i + 1) begin : label
...
module ref_f33m_inv (
  ...
  wire [`WIDTH:0] a0, a1, a2,
                    ...
                    v12, v13, v14, v15, v16,
                    v17, nv2, nv11, nv14;
  wire rst1, rst2, rst3, rst4, done1, done2, done3, done4, dummy;
  reg [4:0] K;
  ...
  always @(posedge clk)
    if (reset) K <= 5'h10;
    else if ((K[4] & rst2) | (K[3] & rst3) | (K[2] & rst4) | (K[1] & done4) | K[0]) K <= K >> 1;
...
module ref_f32m_mult (
  ...
  output reg [`W2:0] c;
  output reg done;
  wire [`WIDTH:0] a0, a1, b0, b1, c0, c1, v1, v2, v3, v4, v5, v6;
  reg mult_reset;
  wire mult_done, p;
  ...
  always @(posedge clk) mult_reset <= reset;
...
module ref_f3m_mux3 (
  ...
  genvar i;
  generate
    for (i = 0; i <= `WIDTH; i = i + 1) begin : label
      assign out[i] = (v1[i] & l1) | (v2[i] & l2) | (v3[i] & l3);
    end
  endgenerate
endmodule
...
module ref_f3m_neg (
  ...
  genvar i;
  generate
    for (i = 0; i <= `WIDTH; i = i + 2) begin : label
...
module ref_f3m_mult (
  ...
  always @(posedge clk) if (done1) C <= z;

  always @(posedge clk)
    if (reset) done <= 0;
    else if (done1) done <= 1;
...
module ref_f3m_cubic (
    input  wire [193:0] in,
    output wire [193:0] out
);
  wire [1:0] w0;
  ref_f3_add a0 (
      in[131:130],
...
module ref_f3m_inv (
  ...
  reg [2*`M:0] i;
  wire [1:0] q;
  wire [`WIDTH+2:0] S1, S2, R1, U1, U2, U3, V1, V2, d1, d2;
  wire don;

  assign d1  = {d[`WIDTH+1:0], 1'b1};
  assign d2  = {1'b0, d[`WIDTH+2:1]};
  assign don = i[0];
  ...
  always @(posedge clk)
    if (reset) begin
      ...
      V <= 0;
      d <= 0;
    end else if (R[`MOST] == 2'b0) begin
      R <= R1;
      U <= U1;
      d <= d1;
    end else if (d[0] == 1'b0) begin
      R <= S2;
      S <= R;
      U <= V2;
      V <= U;
...
module ref_func1 (
  ...
  genvar i;
  generate
    for (i = 0; i <= `WIDTH + 2; i = i + 2) begin : label
      ref_f3_sub s1 (
          S[i+1:i],
          t[i+1:i],
          out[i+1:i]
...
module ref_f3_mult (
  ...
  input [1:0] B;
  output [1:0] C;
  wire a0_local, a1_local, b0_local, b1_local;
  assign {a1_local, a0_local} = A;
  assign {b1_local, b0_local} = B;
  assign C[0] = (~a1_local & a0_local & ~b1_local & b0_local) | (a1_local & ~a0_local & b1_local & ~b0_local);
  assign C[1] = (~a1_local & a0_local & b1_local & ~b0_local) | (a1_local & ~a0_local & ~b1_local & b0_local);
endmodule
